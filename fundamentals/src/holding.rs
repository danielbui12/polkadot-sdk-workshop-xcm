    ...
    pub fn subsume_assets(&mut self, mut assets: AssetsInHolding) {
        ...
        */
        ...
        for (key, value) in assets.fungible.iter_mut() {
            if let Some(existing_amount) = self.fungible.get_mut(key) {
                existing_amount.saturating_accrue(*value);
            } else {
                    ...
                    match asset {
                        Asset { fun: Fungible(amount), id } => {
                            ...
                            */
                            ...
                            let res_amount = if let Some(self_amount) = self.fungible.get_mut(&id) {
                                let min_amount = amount.min(*self_amount);
                                *self_amount -= min_amount;
                                if *self_amount == 0 {
                            ...
                            if res_amount > 0 {
                                taken.subsume(Asset::from((id, res_amount)))
                            }
                        },
