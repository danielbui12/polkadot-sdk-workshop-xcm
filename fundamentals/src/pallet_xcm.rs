...
impl<T: Config> Pallet<T> {
    pub fn do_execute(origin: OriginFor<T>, message: Xcm<()>) -> DispatchResult {
        ...
        T::XcmExecutor::execute(execute_origin, message)
            .map_err(|_| Error::<T>::ExecutorError)?;
        Ok(())
    }
    ...
    pub fn do_send(origin: OriginFor<T>, dest: Location, mut message: Xcm<()>) -> DispatchResult {
        ...
        */
        ...
        let interior: Junctions = origin_location.try_into().map_err(|_| Error::<T>::InvalidOrigin)?;
        ...
        let _msg_id = T::XcmRouter::deliver(ticket)
            .map_err(|_| Error::<T>::RouterError)?;
    ...
    pub fn do_teleport_assets(
        ...
        */
        ...
        reanchored_assets.reanchor(&dest, &context).map_err(|_| Error::<T>::CannotReanchor)?;
